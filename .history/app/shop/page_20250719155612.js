...
export default function Shop() {
  ...
  const categoryFromUrl = searchParams.get("category") || "mens-shoes"; // fallback

  const [products, setProducts] = useState([]);
  const [category, setCategory] = useState(categoryFromUrl);

  useEffect(() => {
    setCategory(categoryFromUrl); // sync state with query
  }, [categoryFromUrl]);
  ...
  return (
    <section className="w-full min-h-screen px-4 md:px-12 pt-20 pb-32 bg-white dark:bg-[#111827] text-black dark:text-white transition-colors duration-500">
      <div className="max-w-7xl mx-auto">
        ...
        ✅ Result: Clicking buttons updates both state and URL Shop auto-fetches
        correct category URLs look like /shop?category=mens-shoes — 🔗 shareable
        And the animated image swap on hover? 💨 Still buttery smooth Let me
        know if you want: Pagination support Filtering (like price or brand) Or
        a "back to top" scroll animation 🌀 You're killin’ it 👟🔥 Ask ChatGPT
        {/* Product Grid */}
